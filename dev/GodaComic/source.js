...
var _Sources = (() => {
  ...
  // src/GodaComic/GodaComic.ts
  var GodaComic_exports = {};
  __export(GodaComic_exports, {
    GodaComic: () => GodaComic,
    GodaComicInfo: () => GodaComicInfo
  });
  ...
  // src/GodaComic/GodaComic.ts
  var BASE_URL = "https://manhuascans.org";
  var GodaComicInfo = {
    version: "1.0.0",
    name: "GodaComic",
  ...
  var GodaComic = class {
    ...
    async getHomePageSections(sectionCallback) {
      ...
      for (const section of sections) {
        ...
        section.items = this.parseSectionItems($2);
        sectionCallback(section);
      }
    }
    async getViewMoreItems(sectionId, metadata) {
      ...
      const response = await this.requestManager.schedule(request, 1);
      const $2 = load(response.data);
      const items = this.parseSectionItems($2);
      const hasNextPage = $2("button:contains('NEXT'), a:contains('NEXT')").length > 0 || $2("span:contains('NEXT')").closest("button").length > 0;
    ...
    parseSectionItems($2) {
      const items = [];
      $2(".trending, a.slicarda, div.pb-2 a").each((_, el) => {
        const anchor = $2(el).is("a") ? $2(el) : $2(el).find("a").first();
    ...
    async getMangaDetails(mangaId) {
      ...
      const $2 = load(response.data);
      let primaryTitle = "";
      const h1 = $2("h1.mb-2").first();
      if (h1 && h1.length) {
        h1.find("span").remove();
        primaryTitle = h1.text().trim();
      }
      if (!primaryTitle) {
        primaryTitle = $2("h2[itemprop=title]").first().text().trim() || "";
      }
      const altTitles = [];
      ...
      const ogImage = $2("meta[property='og:image']").attr("content") || $2("meta[name='og:image']").attr("content");
      let synopsis = $2(".text-medium.line-clamp-4").first().text().trim();
      if (!synopsis) synopsis = $2(".description").first().text().trim();
      let statusText = $2("h1.mb-2 span").first().text().trim();
      if (!statusText) {
        $2(".row.py-1").each((_, el) => {
          const label = $2(el).find(".col-4").first().text().trim();
      ...
      const genres = [];
      $2("div").each((_, el) => {
        const label = $2(el).find("span.font-medium").first().text().trim();
        if (label.startsWith("Genres")) {
          $2(el).find("a").each((_2, g) => {
            const text3 = $2(g).text().trim();
            if (text3) genres.push(text3);
          });
        }
      });
      const tagGroups = [];
    ...
    async getChapters(mangaId) {
      ...
      const allChaptersDiv = $2("#allchapters");
      ...
      const chapters = [];
      $2("#allchapterlist .chapteritem a").each((_, el) => {
        const anchor = $2(el);
        const href = anchor.attr("href") || "";
        ...
        if (mData && cData) {
          ...
          chapterId = parts.length ? parts[parts.length - 1] : void 0;
        }
        if (!chapterId) return;
  ...
  return __toCommonJS(GodaComic_exports);
})();
