...
var _Sources = (() => {
  ...
  // src/Rawkuma/Rawkuma.ts
  var BASE_URL = "https://rawkuma.net";
  var RawkumaInfo = {
    version: "1.1",
    name: "Rawkuma",
  ...
  var Rawkuma = class {
    ...
    async getHomePageSections(sectionCallback) {
      ...
      const popularItems = [];
      $2(".swiper-slide").not(".manga-swipe").each((_, element) => {
        const slide = $2(element);
      ...
      const carouselItems = [];
      $2(".swiper-slide.manga-swipe").each((_, element) => {
        const slide = $2(element);
        const titleAnchor = slide.find("a").first();
        const mangaUrl = titleAnchor.attr("href") || "";
        const mangaIdMatch = mangaUrl.match(/\/manga\/([^/]+)\/?/);
    ...
    async getViewMoreItems(_sectionId, _metadata) {
    ...
    async getSearchResults(query, metadata) {
      ...
      const $lib = load(libResponse.data);
      let nonce = "";
      $lib("script").each((_, el) => {
        const text3 = $lib(el).html() || "";
        const match = text3.match(/"nonce"\s*:\s*"([^"]+)"/);
        if (match && match[1]) {
          nonce = match[1];
          return false;
        }
      ...
      $2("a[href*='/manga/']").each((_, element) => {
        ...
        const image = container.find("img.wp-post-image").first().attr("src") || container.parents("div").find("img.wp-post-image").first().attr("src") || "";
        if (!title || !mangaId) return;
        collectedIds.push(mangaId);
        results.push(App.createPartialSourceManga({ mangaId, image, title }));
      });
      const hasNextPage = $2(`button[onclick*="'page', '${page + 1}'"]`).length > 0;
      return App.createPagedResults({
        results,
        metadata: results.length > 0 && hasNextPage ? { page: page + 1, collectedIds } : void 0
      });
    }
    async getMangaDetails(mangaId) {
      ...
      const rawImage = $2('div[itemprop="image"] img').first().attr("src") || "";
      const image = rawImage.startsWith("/") ? `${BASE_URL}${rawImage}` : rawImage;
      const description = $2('div[itemprop="description"][data-show="true"]').first().text().trim();
      const status = "ONGOING";
      const genres = $2('a[itemprop="genre"]').map((_, el) => $2(el).text().trim()).get();
      let publisher = "";
      const serializationHeading = $2("h4").filter((_, el) => {
        const text3 = $2(el).text().trim().toLowerCase();
        return text3.includes("serialization") || text3.includes("publisher");
      }).first();
    ...
    async getChapters(mangaId) {
      ...
      const $manga = load(response.data);
      const hxGet = $manga("[hx-get*='chapter_list']").first().attr("hx-get") || "";
      ...
      const $2 = load(chapResponse.data);
      const chapters = [];
      $2("div[data-chapter-number]").each((_, element) => {
        const el = $2(element);
        const href = el.find("a").first().attr("href") || "";
        if (!href) return;
        const chapterTitle = el.find("div.flex.flex-row span").first().text().trim();
        ...
        chapters.push(
          App.createChapter({
            ...
            name: chapterTitle,
            chapNum,
            time: dateAttr ? new Date(dateAttr) : /* @__PURE__ */ new Date(),
