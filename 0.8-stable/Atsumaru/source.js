...
var _Sources = (() => {
  ...
  // src/Atsumaru/Atsumaru.ts
  var Atsumaru_exports = {};
  __export(Atsumaru_exports, {
    Atsumaru: () => Atsumaru,
  ...
  var MIN_CHAPTER_OPTIONS = [10, 25, 50, 100];
  ...
  var Atsumaru = class {
        ...
        rows: async () => [
          App.createDUISwitch({
            ...
            value: App.createDUIBinding({
              ...
              set: async (value) => await this.stateManager.store(SHOW_ADULT_KEY, value)
            })
          })
        ]
      });
      ...
      for (const section of data.homePage.sections) {
        ...
        sectionCallback(
          App.createHomeSection({
            ...
            type: section.key === "trending-carousel" ? import_types.HomeSectionType.featured : import_types.HomeSectionType.singleRowNormal,
    ...
    async getSearchTags() {
      const filters = await this.fetchJSON(
      ...
      if (filters.genres.length > 0) {
        sections.push(
          App.createTagSection({
            id: "genres",
            label: "Genres",
            tags: filters.genres.map(
              (genre) => App.createTag({ id: genre.id, label: genre.name })
            )
          })
        );
      }
      if (filters.types.length > 0) {
        sections.push(
          App.createTagSection({
            id: "types",
            label: "Types",
            tags: filters.types.map(
              (type) => App.createTag({ id: `type-${type.id}`, label: type.name })
            )
          })
        );
      }
      if (filters.statuses.length > 0) {
        sections.push(
          App.createTagSection({
            id: "statuses",
            label: "Status",
            tags: filters.statuses.map(
              (status) => App.createTag({ id: `status-${status.id}`, label: status.name })
            )
          })
        );
      }
      ...
      sections.push(
        App.createTagSection({
          ...
          label: "Release Year",
          tags: years.map(
            (year) => App.createTag({ id: `year-${year}`, label: String(year) })
          )
        })
      );
      sections.push(
        App.createTagSection({
          ...
          tags: MIN_CHAPTER_OPTIONS.map(
            (count) => App.createTag({ id: `minchap-${count}`, label: `${count}+ Chapters` })
          )
        })
      );
      sections.push(
      ...
      sections.push(
        App.createTagSection({
          id: "sort",
          label: "Sort By",
          tags: SORT_OPTIONS.map(
            (option) => App.createTag({ id: `sort-${option.id}`, label: option.label })
          )
        })
      );
      return sections;
    ...
    async getSearchResults(query, metadata) {
      ...
      const excludedGenres = (query.excludedTags ?? []).map((tag) => tag.id).filter((id) => /^\d+$/.test(id));
      const filterBy = [];
    ...
    async getMangaDetails(mangaId) {
      ...
      const titles = [manga.title, manga.englishTitle, ...manga.otherNames ?? []].map((title) => title?.trim()).filter((title) => !!title);
      const tags = [];
      ...
      return App.createSourceManga({
        ...
        mangaInfo: App.createMangaInfo({
          ...
          author: manga.authors.map((author) => author.name).join(", ") || void 0,
          desc: manga.synopsis,
    ...
    async getChapters(mangaId) {
      ...
      const sorted = data.chapters.map((chapter) => ({
        chapter,
        group: chapter.scanlationMangaId ? scanlatorMap.get(chapter.scanlationMangaId) ?? "No Group" : "No Group"
      })).sort((a, b) => {
      ...
      return sorted.map(({ chapter, group }, index) => {
        const name = chapter.title.replace(/^((Chapter|Episode|Ch\.?)\s*[\d.]+|#\s*[\d.]+)\s*(\bS\d+\b)?\s*[-:]?\s*/i, "").trim();
    ...
    async getShowAdult() {
      return await this.stateManager.retrieve(SHOW_ADULT_KEY) ?? false;
    }
    async fetchJSON(url) {
    ...
    async fetchMangaPage(mangaId) {
      ...
      const manga = JSON.parse(match[1]).mangaPage;
      await this.stateManager.store(`title-${mangaId}`, manga.title);
      return manga;
    }
    async resolveStaleMangaId(mangaId) {
      const title = await this.stateManager.retrieve(`title-${mangaId}`);
      if (!title) {
    ...
    createPartialManga(item) {
      return App.createPartialSourceManga({
        mangaId: item.id,
        title: item.title,
        image: this.buildThumbnailUrl(item.image),
        subtitle: item.type || void 0
      });
    }
    buildThumbnailUrl(source) {
      const imagePath = typeof source === "string" ? source : source?.posterMedium ?? source?.posterSmall ?? source?.poster;
      if (!imagePath) return "";
  ...
  return __toCommonJS(Atsumaru_exports);
})();
