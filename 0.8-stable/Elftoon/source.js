...
var _Sources = (() => {
  ...
  // src/Elftoon/Elftoon.ts
  var Elftoon_exports = {};
  __export(Elftoon_exports, {
    Elftoon: () => Elftoon,
  ...
  var Elftoon = class {
    ...
    async getHomePageSections(sectionCallback) {
      ...
      const trending = await this.getTrendingSectionItems(void 0);
      const latest = await this.getMangaListItems(void 0, "update");
      const popular = await this.getMangaListItems(void 0, "popular");
      for (const section of sections) {
        switch (section.id) {
    ...
    async getSearchResults(query, metadata) {
      ...
      if (!searchTitle) {
        ...
        return App.createPagedResults({
          ...
          metadata: paged.metadata ? {
            page: paged.metadata.page,
            searchCollectedIds: paged.metadata.collectedIds
          } : void 0
        });
      }
      ...
      this.extractSearchCards($2, ".bs", results, seen);
      this.extractSearchCards($2, ".c-tabs-item__content, .row.c-tabs-item, .utao.styletwo", results, seen);
      const hasNextPage = $2(".pagination a.next, .hpage .r").length > 0;
      return App.createPagedResults({
        results,
        metadata: hasNextPage ? { page: page + 1, searchCollectedIds: Array.from(seen) } : void 0
      });
    }
    async getMangaDetails(mangaId) {
      ...
      if (!title) title = $2("h1.entry-title").first().text().trim();
      if (!title) title = $2("title").first().text().split(" - ")[0]?.trim() ?? mangaId;
      const altTitles = [];
      $2(".alternative").each((_, element) => {
        const text3 = $2(element).text().trim();
        if (text3) altTitles.push(text3);
      });
      let image = $2(".main-info .info-left .thumb img").first().attr("src") || $2(".main-info .info-left .thumb img").first().attr("data-src") || "";
      if (image && !image.startsWith("http")) {
        image = image.startsWith("/") ? `${DOMAIN}${image}` : `${DOMAIN}/${image}`;
      }
      const description = $2(".entry-content p, .entry-content-single p").map((_, el) => $2(el).text().trim()).get().join("\n").trim() || $2(".entry-content, .description-summary").first().text().trim();
      const authors = [];
      ...
      $2(".tsinfo .imptdt").each((_, element) => {
        ...
        if (text3.includes("Status")) {
          const value = $2(element).find("i").text().trim().toLowerCase();
      ...
      let rating = 0;
      const ratingText = $2(".rating .num").first().text().trim();
      if (ratingText) {
    ...
    async getChapters(mangaId) {
      ...
      $2(".eplister ul li").each((_, element) => {
        ...
        const isLocked = href === "#" || chapterLink.attr("data-bs-target") === "#lockedChapterModal" || chapterElement.find('[data-bs-target="#lockedChapterModal"]').length > 0;
        if (!href || isLocked) {
          return;
        }
        const chapterNumText = chapterElement.find(".chapternum").text().trim();
        const chapterTitle = chapterNumText || chapterLink.text().trim();
        let chapterId = href.replace(DOMAIN, "").replace(/^\//, "").replace(/\/$/, "");
    ...
    async getChapterDetails(mangaId, chapterId) {
      const chapterUrl = chapterId.startsWith("http") ? chapterId : `${DOMAIN}/${chapterId.replace(/^\//, "")}`;
      ...
      const match = html3.match(scriptRegex);
      if (match?.[1]) {
        try {
      ...
      if (pages.length === 0) {
        $2(".reading-content img, .reader-area img").each((_, img) => {
          const image = $2(img).attr("data-src") || $2(img).attr("src") || "";
    ...
    async getTrendingSectionItems(metadata) {
      ...
      $2(".swiper-slide .mainslider").each((_, element) => {
        ...
        const mangaId = href.replace(DOMAIN, "").replace(/^\//, "").replace(/^manga\//, "").replace(/\/$/, "");
    ...
    async getMangaListItems(metadata, orderType, forSearch = false) {
      const page = metadata?.page ?? 1;
      const collectedIds = metadata?.collectedIds ?? [];
      const request = App.createRequest({
      ...
      const hasNextPage = $2(".hpage .r, .pagination a.next").length > 0;
      return App.createPagedResults({
        results: items,
        metadata: hasNextPage ? forSearch ? { page: page + 1, searchCollectedIds: Array.from(seen) } : { page: page + 1, collectedIds: Array.from(seen) } : void 0
      });
    }
    extractSearchCards($2, selector, target, seen) {
      $2(selector).each((_, element) => {
        ...
        const subtitle = unit.find(".epxs").first().text().trim() || unit.find(".latestchap").first().text().trim();
        if (title && mangaId && !seen.has(mangaId)) {
          seen.add(mangaId);
          target.push(
            App.createPartialSourceManga({
    ...
    parseDate(raw) {
      ...
      if (!text3) return /* @__PURE__ */ new Date();
      const direct = new Date(text3);
      if (!isNaN(direct.getTime())) {
        return direct;
      }
      const now = Date.now();
      const rel = text3.toLowerCase();
      const amount = parseInt(rel.match(/\d+/)?.[0] ?? "0", 10);
      if (rel.includes("minute")) return new Date(now - amount * 6e4);
      if (rel.includes("hour")) return new Date(now - amount * 36e5);
      if (rel.includes("day")) return new Date(now - amount * 864e5);
      if (rel.includes("week")) return new Date(now - amount * 6048e5);
      return /* @__PURE__ */ new Date();
    }
  };
  return __toCommonJS(Elftoon_exports);
})();
