...
var _Sources = (() => {
  ...
  // src/Projectsuki/Projectsuki.ts
  var Projectsuki_exports = {};
  __export(Projectsuki_exports, {
    Projectsuki: () => Projectsuki,
    ProjectsukiInfo: () => ProjectsukiInfo
  });
  ...
  // src/Projectsuki/Projectsuki.ts
  var DOMAIN = "https://projectsuki.com";
  var ProjectsukiInfo = {
    version: "1.0.0",
    name: "Projectsuki",
  ...
  var Projectsuki = class {
    ...
    async getMangaDetails(mangaId) {
      ...
      const primaryTitle = $2("h2[itemprop=title]").first().text().trim() || "";
      const altTitles = [];
      $2(".row.py-1").each((_, el) => {
        const label = $2(el).find(".col-4").first().text().trim();
        if (label.startsWith("Alternative titles")) {
          const text3 = $2(el).find(".col-8").text().trim();
          if (text3) {
            altTitles.push(...text3.split(",").map((s) => s.trim()).filter(Boolean));
          }
        }
      });
      let thumbnailUrl = $2(".img-thumbnail").first().attr("src") || "";
      if (thumbnailUrl && thumbnailUrl.startsWith("/")) {
        thumbnailUrl = DOMAIN + thumbnailUrl;
      }
      const synopsis = $2(".description").text().trim();
      let statusText = "";
      $2(".row.py-1").each((_, el) => {
        const label = $2(el).find(".col-4").first().text().trim();
        if (label.startsWith("Status")) {
          statusText = $2(el).find(".col-8").text().trim();
        }
      });
      let status = "UNKNOWN";
      ...
      $2(".row.py-1").each((_, el) => {
        ...
        if (label.startsWith("Genre")) {
          $2(el).find(".col-8 a").each((_2, g) => {
            genres.push($2(g).text().trim());
          });
        }
      });
      const tagGroups = [];
    ...
    async getChapters(mangaId) {
      ...
      const $2 = load(response.data);
      const chapters = [];
      let rows = $2("table tbody tr");
      if (rows.length === 0) {
        rows = $2("tr.row.mx-0").filter((_, r) => $2(r).find("td a").length > 0);
      }
      rows.each((_, row) => {
        const anchor = $2(row).find("td a").first();
        const href = anchor.attr("href") || "";
        const match = href.match(/\/read\/\d+\/(\d+)/);
        const chapterId = match ? match[1] : void 0;
        if (!chapterId) return;
        ...
        let publishDate;
        const dateSpan = $2(row).find("span[itemscope][itemtype*=dateCreated]").first();
        const dateTitle = dateSpan.attr("title") || dateSpan.text().trim();
        if (dateTitle) {
          const parts = dateTitle.split("-");
          if (parts.length === 3) {
            publishDate = /* @__PURE__ */ new Date(`${parts[2]}-${parts[1]}-${parts[0]}`);
          } else {
            publishDate = new Date(dateTitle);
          }
        }
        const lang = $2(row).find("td").eq(1).text().trim();
    ...
    async getSearchResults(query, metadata) {
      ...
      const items = [];
      $2(".browse").each((_, el) => {
        const titleAnchor = $2(el).find(".details h4 a").first();
        const href = titleAnchor.attr("href") ?? "";
        const idMatch = href.match(/\/book\/(\d+)/);
        const mangaId = idMatch ? idMatch[1] : void 0;
        if (!mangaId) return;
        const title = titleAnchor.text().trim();
        let imageUrl = $2(el).find(".mr-2 img").first().attr("src") ?? "";
    ...
    async getHomePageSections(sectionCallback) {
      ...
      for (const section of sections) {
        ...
        if (section.id === "popular") {
          $2(".trending").each((_, el) => {
            const anchor = $2(el).find("a").first();
            const href = anchor.attr("href") ?? "";
            const idMatch = href.match(/\/book\/(\d+)/);
            const mangaId = idMatch ? idMatch[1] : void 0;
            if (!mangaId) return;
            const img = $2(el).find("img").first();
            items.push(App.createPartialSourceManga({
              mangaId,
              image: img.attr("src") ?? "",
          ...
          $2(".item").each((_, el) => {
            const anchor = $2(el).find(".mr-2 a").first();
            const href = anchor.attr("href") ?? "";
            const idMatch = href.match(/(\d+)/);
            const mangaId = idMatch ? idMatch[1] : void 0;
            if (!mangaId) return;
            const img = anchor.find("img").first();
            const titleAnchor = $2(el).find(".title a").first();
            const chapterAnchor = $2(el).find(".pages a").first();
            items.push(App.createPartialSourceManga({
              mangaId,
              image: img.attr("src") ?? "",
  ...
  return __toCommonJS(Projectsuki_exports);
})();
